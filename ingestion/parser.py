...
def extract_choices(value: Any, floor: int | None = None, path: str = "") -> list[Json]:
    ...
    if isinstance(value, dict):
        for key, child in value.items():
            ...
            found.extend(extract_choices(child, floor, child_path))
    elif isinstance(value, list):
        for index, child in enumerate(value):
            found.extend(extract_choices(child, floor, f"{path}[{index}]") )
    return found
...
def parse_run_data(raw: Json, source_path: str | None = None) -> Json:
    ...
    for floor, history_item in enumerate(history, start=1):
        nodes = player_nodes(history_item)
        if not nodes:
            continue
        primary = nodes[0]
        choices = [choice for node in nodes for choice in extract_choices(node, floor)]
        floor_choices.extend(choices)
        floor_history.append({
            "floor": floor,
            "node_type": first(primary, "map_point_type", "room_type", "node_type"),
            "encounter": first(primary, "encounter", "encounter_id", "monster", "enemy"),
            "event": first(primary, "event", "event_id"),
    ...
    result = "win" if explicit_win is True else "loss" if death_cause else "unknown"
    ...
    return {
        ...
        "context": {
            "character": first(raw, "character", "character_id", "player_character", default=first(stats, "character", "character_id", "character_name")),
